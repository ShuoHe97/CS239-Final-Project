...
TEST(ChSensorManager, box_intersect) {
    ...
    auto boxA = std::make_shared<ChBodyEasyBox>(1, 1, 1,  // x,y,z size
                                                100,      // density
                                                true,     // collide enable?
                                                true);    // visualization?
    ...
    auto c = std::make_shared<ChSensorManager>(&mphysicalSystem);
    // c->SetRenderDimensions(5, 5);

    auto cam = std::make_shared<ChCameraSensor>(
        boxA, 50, chrono::ChFrame<double>({-8, 0, 3}, Q_from_AngAxis(0, {1, 0, 0})), 1, 1, 1, 1);
...
TEST(ChSensorManager, sphere_intersect) {
    ChSystemNSC mphysicalSystem;
    auto sphere = std::make_shared<ChBodyEasySphere>(1,      // x,y,z size
                                                     100,    // density
                                                     true,   // collide enable?
                                                     true);  // visualization?
    ...
    auto c = std::make_shared<ChSensorManager>(&mphysicalSystem);
    auto cam = std::make_shared<ChCameraSensor>(
        sphere, 50, chrono::ChFrame<double>({-8, 0, 3}, Q_from_AngAxis(0, {1, 0, 0})), 1, 1, 1, 1);
    cam->SetName("Camera Sensor");
    c->AddSensor(cam);
...
TEST(ChSensorManager, mesh_intersect) {
    ChSystemNSC mphysicalSystem;
    auto dummy = std::make_shared<ChBodyEasySphere>(1,      // x,y,z size
                                                    100,    // density
                                                    true,   // collide enable?
                                                    true);  // visualization?
    ...
    auto c = std::make_shared<ChSensorManager>(&mphysicalSystem);
    auto cam = std::make_shared<ChCameraSensor>(
        dummy, 50, chrono::ChFrame<double>({-8, 0, 3}, Q_from_AngAxis(0, {1, 0, 0})), 1, 1, 1, 1);
    cam->SetName("Camera Sensor");
    c->AddSensor(cam);
