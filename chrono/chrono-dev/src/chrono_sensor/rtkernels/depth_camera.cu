...
RT_PROGRAM void pinhole_camera() {
    ...
    // // prd_radiance.type = CAMERA;
    // // prd_radiance.first_distance = -1.;
    // // prd_radiance.distance = 0;
    //
    // prd_radiance.importance = 1.f;
    // prd_radiance.depth = 0;
    // prd_radiance.type = CAMERA;
    ...
    // output_buffer[launch_index] = make_float4(.5);  // make_color(prd_radiance.color);
    uchar4 color = make_color(prd_radiance.color);
    float3 rel_forward = make_float3(1, 0, 0);
    float3 rel_left = make_float3(0, 1, 0);
    float3 rel_up = make_float3(0, 0, 1);

    if (prd_radiance.first_distance < scene_epsilon) {
        prd_radiance.first_distance = 20.0;
    }

    // float3 point = ray_direction * prd_radiance.distance;
    float3 point =
        normalize(rel_forward - d.x * rel_left * h_factor + d.y * rel_up * v_factor) * prd_radiance.first_distance;
    // if (point.x > 1e5 || point.y > 1e5 || point.z > 1e5) {
    //     printf("Point is massively large...gotta figure out why!\n");
    // }
    // output_buffer[launch_index] = make_float4(point, color);
    output_buffer[launch_index] = make_float4(point, color);
}
